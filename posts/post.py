...
# Mock friendship data for testing privacy settings
# In a real implementation, this would be in your database model
FRIENDSHIPS = {
    # username: [list of friend usernames]
    'user1': ['user2', 'user3'],
    'user2': ['user1'],
    'user3': ['user1'],
...
def are_friends(user1, user2):
    ...
    # In a real implementation, you would query your Friendship model
    # This is a mock implementation for testing
    return (user1 in FRIENDSHIPS.get(user2, []) or 
            user2 in FRIENDSHIPS.get(user1, []))
...
def create_post(request):
    """Create a new post with support for image uploads and privacy settings"""
    ...
    try:
        ...
        privacy = request.POST.get('privacy', 'public')  # Default to public if not specified
        ...
        log(f"Post data - Content: {content}, Type: {post_type}, Privacy: {privacy}, Has image: {image is not None}")
        ...
        new_post = {
            ...
            "privacy": privacy,  # Add the privacy setting
            "comments": []
        }
...
def get_posts(request):
    """Get all posts for the newsfeed, respecting privacy settings"""
    try:
        # If no posts exist, create a starter post
        if not USER_POSTS:
            USER_POSTS.append({
                "id": uuid.uuid4().hex[:8],
                "post_type": "text",
        ...
        for post in USER_POSTS:
            ...
            # Determine if the current user can see this post
            if (
                privacy == "public" or  # Public posts visible to all
                post_author == current_user or  # User's own posts
                (privacy == "friends" and are_friends(current_user, post_author))  # Friends-only posts
            ):
                visible_posts.append(post)
        
        log(f"Returning {len(visible_posts)} visible posts out of {len(USER_POSTS)} total")
        return JsonResponse({"posts": visible_posts})
    except Exception as e:
    ...
    try:
        ...
        for post in USER_POSTS:
            if str(post["id"]) == str(post_id):
                # Check if the user has access to this post before liking
                post_author = post.get("author", "")
                privacy = post.get("privacy", "public")
                current_user = request.user.username
                ...
                if not (
                    privacy == "public" or
                    post_author == current_user or
                    (privacy == "friends" and are_friends(current_user, post_author))
                ):
                    return JsonResponse({"error": "Not authorized to access this post"}, status=403)
...
def comment_post(request, post_id):
    ...
    try:
        ...
        for post in USER_POSTS:
            if str(post["id"]) == str(post_id):
                # Check if the user has access to this post before commenting
                post_author = post.get("author", "")
                privacy = post.get("privacy", "public")
                current_user = request.user.username
...
def delete_post(request, post_id):
    ...
    try:
        ...
        for i, post in enumerate(USER_POSTS):
            if str(post["id"]) == str(post_id):
                # Check if the user is the author or admin/moderator
                is_author = post["author"] == request.user.username
                is_moderator = getattr(request.user, 'role', '') in ['admin', 'moderator']
